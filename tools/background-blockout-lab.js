...
(async()=>{
  ...
  const previous=clearing==='original',simple=!archive&&!previous&&clearing!=='crisp';
  const candidate=archive?'gatherer-blockout-v2':simple?'gatherer-simple-v1':'gatherer-near-v3';
  const base='art-source/knight-rush-backgrounds/'+(archive?'gatherer-blockout-v2':'gatherer-near-v3')+'/';
  ...
  const approvedNav=document.createElement('nav');approvedNav.style.cssText='padding:14px 24px;background:#242c30';
  approvedNav.innerHTML='<a href="KnightRush.html?backgroundlab=1">Candidate: Simple clearing</a> · <a href="KnightRush.html?backgroundlab=1&clearing=crisp">Approved: Gatherer clearing</a> · <a href="KnightRush.html?backgroundlab=1&scene=basalt-forge">Approved: Basalt Hearth / Borin</a> · <a href="KnightRush.html?backgroundlab=1&scene=autumn-caravan">Approved: Autumn Caravan</a> · <a href="KnightRush.html?backgroundlab=1&scene=mossy-inn">Approved: Mossy Oak Inn</a> · <a href="RoadCreatorLab.html">Road Creator Lab</a>';
  document.body.appendChild(approvedNav);
  const approval=archive?'spatial-layout-only':previous?'archived-variant':simple?'candidate':reference.status;
  const illustrationURL='assets/encounters/mushroom-clearing'+(previous?'':simple?'-simple':'-crisp')+'.png';
  ...
  const root=document.createElement('main');root.id='blockout-root';root.innerHTML=`
    <div class="bo-kicker">KNIGHT RUSH CUTSCENE / BACKGROUND LAB / ${archive?'SPATIAL BLOCKOUT V2':previous?'ARCHIVED VARIANT':simple?'SIMPLIFIED CANDIDATE':'APPROVED REFERENCE'}</div>
    <h1>The Gatherer's Clearing</h1>
    <p>${archive?'Archived spatial model. Grey materials and tree volumes are placeholders.':previous?'Earlier image retained for comparison; not the locked cutscene reference.':simple?'Simplified broad-plane clearing trial. Same live character, sunlight and mobile image budget. Awaiting user approval.':'User-approved crisp clearing and scene-lit live gatherer. Environment quality reference, not a requirement for forests or this UI.'}</p>
    <div class="bo-controls">
      ...
      ${archive?'':`<button id="bo-playtest">Play in-game encounter</button><a href="KnightRush.html?backgroundlab=1&clearing=${previous||simple?'crisp':'simple'}">${previous||simple?'Compare approved reference':'Compare simplified candidate'}</a><a href="KnightRush.html?backgroundlab=1&clearing=original">Original archive</a><span>${previous?'Previous image':simple?'Simplified candidate':'Approved crisp reference'} · one selected plate</span>`}
  ...
  if(!archive){
    ...
    const playURL=()=> 'KnightRush.html?backgroundplaytest=1&lighting='+opts.lighting+'&clearing='+(previous?'original':simple?'simple':'crisp');
