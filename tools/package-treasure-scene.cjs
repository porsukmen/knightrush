...
(async()=>{
 ...
 const root=path.resolve(__dirname,'..'),dir=path.join(root,'art-source/knight-rush-backgrounds/treasure-grove-v1');
 fs.mkdirSync(dir,{recursive:true});fs.copyFileSync(source,path.join(dir,'generated.png'));
 const img=await loadImage(source);
 for(const [suffix,w,h]of [['',1120,1400],['-mobile',768,960]]){
  ...
  fs.writeFileSync(path.join(root,'assets/encounters/treasure-grove-v1'+suffix+'.png'),c.toBuffer('image/png'));
