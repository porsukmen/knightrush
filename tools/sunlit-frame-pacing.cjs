...
(async()=>{
 ...
 try{for(const [device,width,height,dpr]of [['desktop',1280,900,1],['phone-emulation',390,844,2]]){
  ...
  for(const theme of ['disco','bloodwood'])for(const turn of [false,true]){
   await run(`Math.random=(()=>{let seed=731;return()=>((seed=Math.imul(seed,1664525)+1013904223)>>>0)/4294967296;})();
    ...
    startRoadLabCase(ROAD_LAB_CASES.findIndex(c=>c.theme===${JSON.stringify(theme)}));
    if(${turn}){
   ...
   const result={device,theme,turn,...data};results.push(result);console.log(JSON.stringify(result));
   await page.screenshot({path:path.join(out,`${label}-${device}-${theme}-${turn?'turn':'straight'}.png`)});
  }
