...
function normalFixture(content,side=null){
 for(let seed=0;seed<100;seed++){
  ...
  for(const n of journeyRoute.nodes)for(const e of n.out){
   const slot=e.events.find(s=>JOURNEY_ROAD_EVENTS[s.definition].kind==='normal'&&journeyNormalContent(s)===content&&(side===null||journeyNormalSide(s)===side));if(!slot)continue;
...
function enterNormal(){handleAction(journeyNormalSide(journeyRoadEventSession.slot)>0?'right':'left');update(.25);}
...
for(const content of ['chest','well']){
 ...
 assert.equal(run('mode'),content==='chest'?'lockpicking':'wishingwell');
 ...
 run(content==='chest'?'beginLockpickingRound()':'beginWishingWellThrow()');
 const setup=run(content==='chest'?'JSON.stringify(lockpickGame.pins)':'JSON.stringify(wellGame.setup)');
 run(content==='chest'?'lockpickGame.phase="result";lockpickGame.won=true;beginLockpickingRound()':'wellGame.phase="result";wellGame.result={tier:"EXCELLENT"};beginWishingWellThrow()');
 assert.equal(run('mode'),'run');assert.equal(run('journeyRoadEventSession'),null);assert.equal(run('gold-goldBefore'),content==='chest'?20:12);
 assert.equal(run('JSON.stringify([dist,runDistance,roadScroll,journeyRoute.activeEdge])'),run('frozen'));
 run('update(1/60)');assert(run('dist')>JSON.parse(run('frozen'))[0]);
 run(`slot=normalFixture('${content}');acceptNormal();`);run(content==='chest'?'beginLockpickingRound()':'beginWishingWellThrow()');
 assert.equal(run(content==='chest'?'JSON.stringify(lockpickGame.pins)':'JSON.stringify(wellGame.setup)'),setup,'Seeded challenge');
 run(`goldBefore=gold;${content==='chest'?'leaveLockpicking()':'leaveWishingWell()'};`);assert.equal(run('gold-goldBefore'),0);
...
run(`slot=normalFixture('chest');globalThis.stale=journeyRoadEventSession.context;acceptNormal();startJourneyWithSeed(12);`);
assert.equal(run('lockpickGame'),null);assert.equal(run("stale.finish({status:'completed'})"),false);
...
for(const content of ['mushrooms','chest','well'])for(const curved of [false,true]){
...
for(const side of [-1,1])for(const offset of [-24,0,15]){
 ...
 assert.equal(run('mode'),'run');
 run(`handleAction(journeyNormalSide(slot)>0?'right':'left');handleAction('choice1');`);
...
console.log('JOURNEY_NORMAL_EVENTS_OK directional swipe/window/debounce/refusal; run-local quest/swept pickup/delivery; new play/replay/new seed/abandon reset; seeded chest/well; frozen dialogue/resume; once-only reward; standalone; 12 renders');
