...
(async()=>{
 const src=process.argv[2];if(!src)throw Error('Pass generated plate');
 const root=path.resolve(__dirname,'..'),dir=path.join(root,'art-source/knight-rush-backgrounds/tavern-games-v1');
 ...
 for(const [suffix,w,h]of [['',1086,1448],['-mobile',576,768]]){
  ...
  fs.writeFileSync(path.join(root,'assets/encounters/tavern-games-v1'+suffix+'.png'),c.toBuffer('image/png'));
