...
(async()=>{
 ...
 try{
  ...
  let inputs=0;
  for(const direction of [-1,1])for(const offset of [-50,-15,3.9,9.9])for(const touch of [false,true]){
  ...
  const missed=await run('turnInputFixture(1,3.9,true)');
  ...
  console.log('JOURNEY_TURN_INPUT_BROWSER_OK '+JSON.stringify({inputs,keyboardAndTouch:true,missedDiscoStable:true,screenshots:out}));
