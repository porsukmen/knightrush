...
run('const roadStage=loop,roadKills=bossKills,roadPack=pack,roadHp=player.currentHealthUnits;const roadTrees=roadsideScenery.map(t=>t.z);');
...
assert(run('loop===roadStage&&bossKills===roadKills&&pack===roadPack+1&&player.currentHealthUnits===roadHp'));
