...
(async()=>{
 ...
 try{for(const [device,width,height,dpr]of [['desktop',775,1000,1],['phone',390,844,2]]){
  ...
  const run=code=>p.evaluate(code=>(0,eval)(code),code),ready=()=>p.waitForFunction(()=>mode==='tavernslide'?(window.KRTavernSlide&&window.KREventVisuals?.peek('tavern-slide')):(window.KRTavernGames&&window.KREventVisuals?.peek('tavern-games')),null,{polling:50});
  ...
  for(const [id,start]of Object.entries(starts)){
   ...
   if(id==='slide'){
    ...
    await tap('ARM_WRESTLE_START_BTN');assert.equal(await run('armWrestleGame.phase'),'playing');
    for(const k of [.05,.5,.95]){await run(`armWrestleGame.power=${k};`);await shot(id+'-pressure-'+k);}
    await run("armWrestleGame.power=.5;for(let i=0;i<20&&armWrestleGame.phase==='playing';i++)armWrestleSwipe('right');for(let i=0;i<180;i++)updateArmWrestling(1/60);");
    assert.equal(await run('armWrestleGame.result.win'),true);
   }else if(id==='dice'){
    await tap('DICE_REPLAY_BTN');assert.equal(await run('diceGame.phase'),'rolling');await shot(id+'-rolling');await run('updateDiceGuess(1);');await shot(id+'-choose');
    await tap('DICE_HIGHER_BTN');assert.equal(await run('diceGame.phase'),'revealing');await run('updateDiceGuess(.3);');await shot(id+'-reveal');await run('updateDiceGuess(1.3);');
    assert.equal(await run('diceGame.result.playerTotal'),await run('diceGame.playerDice.reduce((a,b)=>a+b,0)'));
   }else if(id==='queen'){
    ...
    await tap('FIND_QUEEN_CARD_RECTS[findQueenGame.cards.find(c=>c.role===\'QUEEN\').slot]');await run('updateFindQueen(.6);');assert.equal(await run('findQueenGame.won'),true);
   }else{
    await tap('DRINK_START_BTN');assert.equal(await run('drinkGame.phase'),'preview');await shot(id+'-preview');
    await run("for(let i=0;i<180;i++)updateDrinkingContest(1/60);");await shot(id+'-tracing');
    await run("drinkGame.phase='tracing';beginDrinkTrace(drinkGame.pattern.points[0]);for(const pt of drinkGame.pattern.points.slice(1))moveDrinkTrace(pt);endDrinkTrace();");
    assert.equal(await run('drinkGame.phase'),'roundResult');await shot(id+'-round');
    await run("drinkGame.wins=2;advanceDrinkingContest();");assert.equal(await run('drinkGame.phase'),'complete');
   }
  ...
  for(const lit of [false,true]){await run(`g.save();g.setTransform(1,0,0,1,0,0);g.fillStyle='#a99979';g.fillRect(0,0,cvs.width,cvs.height);g.translate(100,20);KRTavernGames.actor(130,370,1.7,'duke',${lit});g.restore();`);await p.screenshot({path:path.join(out,device+'-actor-'+(lit?'lit':'neutral')+'.png')});}
  ...
  const report=await run('KREventVisuals.report()');assert.equal(report.tier,device==='phone'?'mobile':'standard');assert(report.reservedBytes<=report.budgetBytes);assert.equal(report.active,await run("mode==='tavernslide'?'tavern-slide':'tavern-games'"));
  ...
  // Fixed limb lengths throughout the pin arc (not just attractive end poses).
  assert(await run(`(()=>{for(let k=0;k<=1;k+=.01){const angle=(k-.5)*2.5,a={x:330,y:660},b={x:249+Math.sin(angle)*106,y:456-Math.cos(angle)*106},e=KRTavernGames.elbowFor(a,b,166,185);if(Math.abs(Math.hypot(e.x-a.x,e.y-a.y)-166)>.001||Math.abs(Math.hypot(e.x-b.x,e.y-b.y)-185)>.001)return false;}return true;})()`));
  ...
  await p.evaluate(()=>{const d=Object.getOwnPropertyDescriptor(HTMLImageElement.prototype,'src');window.restoreTavernImageSource=()=>Object.defineProperty(HTMLImageElement.prototype,'src',d);Object.defineProperty(HTMLImageElement.prototype,'src',{...d,set(value){d.set.call(this,String(value).includes('tavern-games-v1')?'assets/encounters/intentionally-missing-tavern-audit.png':value);}});});
  await run('startDiceGuess();');
  await p.waitForFunction(()=>window.KRCutscenes?.report().state==='fallback',null,{polling:50});await shot('image-fallback');await tap('DICE_REPLAY_BTN');assert.equal(await run('diceGame.phase'),'rolling');await tap('MINIGAME_BACK_BTN');assert.equal(await run('mode'),'minigames');
