...
(async()=>{
 ...
 try{
  ...
  await page.goto(pathToFileURL(path.resolve('KnightRush.html')).href+'?merchantlab=1');
  await page.waitForFunction(()=>document.querySelector('canvas')?.dataset.bootReady==='1');
  await page.waitForFunction(()=>window.KRAutumnCaravan&&KRCutscenes?.report().state==='ready');
  ...
  await tap(150,708);assert.equal(await run('gold'),100);await tap(150,708);assert.equal(await run('gold'),100);
  await tap(240,787);assert.equal(await run('mode'),'merchant');
  ...
  await tap(120,752);assert.equal(await run('gold'),200);
  await tap(350,752);assert.equal(await run('gold'),120);assert.equal(await run('relics.filter(Boolean).length'),0);
  await tap(240,787);assert.equal(await run('mode'),'debugcfg');
  await tap(396,359);assert.equal(await run('mode'),'merchant');
  const beforeReroll=await run('merchantShop.stock[0].id');
  await tap(365,708);assert.equal(await run('gold'),110);
  await tap(365,708);assert.equal(await run('gold'),110);
